...
def generate_pseudo_data(fold,timestamp,gen_no):
    ...
    if args.ex_type == "Main":
        generator.load_state_dict(torch.load('GAN/Model/G_model_{}_final.pth'.format(fold), map_location=args.device))
        discriminator.load_state_dict(torch.load('GAN/Model/D_model_{}_final.pth'.format(fold), map_location=args.device))
    ...
    [train_x, train_edge_index, train_label] = load_graph_data(fold=fold, data_filenames=real_filenames)
    ...
    while flag:
        ...
        [gen_data, gen_edge, gen_label] = load_fake_data(fold=fold, data_filenames=real_filenames, fixed_noise=noise)
    ...
    [batch_real_x, batch_real_edge_index, batch_real_label] = load_real_data(fold=fold, data_filenames=real_filenames)
    ...
    [_, real_edge, _] = load_graph_data(fold=fold, data_filenames=real_filenames)
...
def test(fold, gen_no):
    ...
    [test_data, test_edge, test_label] = load_graph_data(fold=fold, data_filenames=test_filenames)
...
if __name__ == "__main__":
    ...
    #         f.write('std,{:.2f},{:.2f},{:.2f},{:.2f}'.format(np.nanstd(total_result[0], ddof=1) * 100, np.nanstd(total_result[1], ddof=1) * 100, np.nanstd(total_result[2], ddof=1) * 100, np.nanstd(total_result[3], ddof=1) * 100))
            ...
            for f1 in range (1,6):
                generate_pseudo_data(f1,args.timestamp, 1)
