...
def generate_pseudo_data(fold, timestamp, gen_no):
    ...
    generator.load_state_dict(
        torch.load('GAN/Model/mrmr_seed_{}_fix/G_model_{}_final.pth'.format(args.seed, fold), map_location=args.device)
    )
    discriminator.load_state_dict(
        torch.load('GAN/Model/mrmr_seed_{}_fix/D_model_{}_final.pth'.format(args.seed, fold), map_location=args.device)
    )
    ...
    if not os.path.isdir('GAN/Pseudo_data/model{}/'.format(args.timestamp)):
        os.makedirs('GAN/Pseudo_data/model{}/'.format(args.timestamp))
...
def train_classifier(fold, train_x, train_edge_index, train_label, gen_no):
    ...
    classifier.load_state_dict(
        torch.load('GCN/Model/model_seed_{}/GCN_model_{}_init.pth'.format(args.seed, fold), map_location=args.device),
        strict=True
    )
    ...
    if not os.path.isdir('GAN/Results_dgan/{}/model{}'.format(args.edge_type, args.timestamp)):
        os.makedirs('GAN/Results_dgan/{}/model{}'.format(args.edge_type, args.timestamp))
    ...
    for epoch in range(1, 2001):
        ...
        for step in range(step_size + 1):
            if step == step_size:
                final_batch = train_x.shape[0] - (step_size * args.batch_size)
            ...
            out_stack.extend(np.argmax(C_real_logits.cpu().detach().tolist(), axis=1))
            label_stack.extend(train_label[batch_mask].cpu().detach().tolist())
        ...
        print("[Train] [Epoch:{}] [Time:{:.1f}] [Loss C:{:.4f}] [ACC C:{:.4f}] [SEN C:{:.4f}] [SPEC C:{:.4f}]".format(
            epoch, time.time() - epoch_time, sum(c_loss) / step_size, C_acc, C_sen, C_spec
        ))
...
def test(fold, gen_no):
    ...
    total_info['acc'] = accuracy(out_stack, label_stack)
    total_info['sen'] = sensitivity(out_stack, label_stack)
    total_info['spec'] = specificity(out_stack, label_stack)
    total_info['f1'] = sklearn.metrics.f1_score(label_stack, out_stack)

    log = "[Test] time [{:.1f}s]".format(time.time() - epoch_time) + \
          " | loss [{:.3}] acc [{:.3}] sen [{:.3f}] spec [{:.3f}] f1 [{:.3f}]".format(
              total_info['loss'], total_info['acc'], total_info['sen'], total_info['spec'], total_info['f1']
          )
    print(log)

    out = {key: total_info[key] for key in eval_list}

    path_save_info = 'GAN/Results_dgan/{}/model{}'.format(args.edge_type, args.timestamp) + os.path.sep + "train_info{}_{}.csv".format(args.timestamp, fold)
    with open(path_save_info.replace(".csv", "_test_{}.csv".format(gen_no)), "a") as f:
        log_line = ",".join([str(out[key]) for key in out.keys()]) + "\n"
        f.write(log_line)
