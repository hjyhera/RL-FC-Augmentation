...
def train(fold):
    ...
    train_log = "##### Training start! ###### \t timestamp: {} \t fold : {} \t lr : {} \t wd : {}".format(args.timestamp, fold, args.lr, args.c_weight_decay)
    print(train_log)

    # load train data
    train_filenames = load_data_list(fold=fold, data_type="train")
    [train_data, train_edge, train_label] = load_graph_data(fold=fold, data_filenames=train_filenames)
    ...
    for epoch in range(1,2001):
        ...
        for step in range(step_size + 1):
            ...
            [train_data, train_edge, train_label] = load_graph_data(fold=fold, data_filenames=train_filenames[batch_mask])
