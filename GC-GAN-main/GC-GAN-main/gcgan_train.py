...
def train_gan(fold,gen_no):
    ...
    # generator.load_state_dict(torch.load('GraphAE/Model/model{}/GAE_model_{}_final.pth'.format(graphae_ts, fold), map_location=args.device), strict=False)
    # discriminator.load_state_dict(torch.load('GCN/Model/model{}/GCN_model_{}_final.pth'.format(gcn_ts, fold), map_location=args.device), strict=False)

    generator.load_state_dict(torch.load('GAN/Model/G_model_{}_final.pth'.format(fold), map_location=args.device), strict=False)
    discriminator.load_state_dict(torch.load('GAN/Model/D_model_{}_final.pth'.format(fold), map_location=args.device), strict=False)
    ...
    [_,_, train_label] = load_real_data(fold=fold, data_filenames=real_filenames)
    ...
    for epoch in range(1,1001):
        ...
        for step in range(step_size + 1):
            ...
            [train_data, train_edge, train_label] = load_real_data(fold=fold, data_filenames=real_filenames[batch_mask], data_type="train")
            ...
            [gen_data, gen_edge, gen_label] = load_fake_data(fold=fold, data_filenames=real_filenames[batch_mask],fixed_noise=noise)
...
if __name__ == "__main__":
    ...
    for i in range(1, args.gen_no + 1):
        for f1 in range(1, 6):
            # save log
            path_save_info = 'GAN/Results_gan/model{}'.format(args.timestamp) + os.path.sep + "train_info{}_{}.csv".format(args.timestamp, f1)
            train_gan(f1, i)