...
def load_topology(topology_dir: str):
    ...
    for fold in range(1, 6):
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"Cannot find topology file: {file_path}")
...
def precompute_rewards(topologies, synthetic_data, model_alignment_root, discriminators, device, save_path=None,
    ...
    def comp_fidelity_logodds(fidelity, mat, topo):
        with torch.no_grad():
            logits, _ = fidelity(mat, topo)           # [NC_r, MDD_r, NC_f, MDD_f]
    ...
    alignment_raw_mse = {f: [] for f in range(1, 6)}    
    fidelity_raw_logodds = {f: [] for f in range(1, 6)}
    ...
    for fold in range(1, 6):
        ...
        for flat_vec, lbl in synthetic_data[fold]:
            ...
            d_mse = comp_alignment(alignment, mat, lbl, topo)               
            m_lo  = comp_fidelity_logodds(discriminators[fold], mat, topo) 
