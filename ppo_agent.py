...
class PPO:
    def __init__(self, actor, critic, config, device=None, logger=None):
        ...
        # === Critic baseline (항상 사용) ===
        self.train_critic = True  # Critic을 baseline으로 사용하므로 항상 학습
        self.critic_coef = float(getattr(config, "critic_coef", 0.5))  # Critic loss 가중치
    ...
    def store_transition(self, state, action, reward, done, logp, old_logits, h_last, info=None):
        ...
        self.buffer.append({
            "state":      state.detach().cpu(),      # [1,N,D]
            "action":     action.detach().cpu(),     # [N]
            "reward":     float(reward),             # scalar (전역 평균)
    ...
    def learn(self, fold=None, step_idx=None, grad_clip=None, eps=1e-8):
        ...
        for epoch in range(self.n_epochs):
            ...
            for start in range(0, T, mb_size):
                ...
                a_mb   = actions[idx]                 # [M,N]
                old_mb = old_logits_stack[idx]        # [M,N,2]
                r_mb   = rewards[idx]                 # [M] (전역 평균)
                h_mb   = h_lasts[idx]                 # [M,H]
                r_sample_mb = per_sample_rewards[idx]  # [M,N] ✅ 샘플별 보상!

                # === Critic forward (baseline 계산) ===
                with torch.no_grad():
                    V_old = self.critic(h_mb)  # [M]
                
                # Critic을 detach하지 않고 계산 (gradient 필요)
                V_new = self.critic(h_mb)  # [M]
                
                # === Per-token advantage with Critic baseline ===
                # r_sample_mb: [M, N] - 각 샘플의 보상
                # V_old: [M] - 전역 baseline
                # Advantage: r_sample - V (broadcasting)
                with torch.no_grad():
                ...
                # === Critic loss (MSE with 전역 평균 보상) ===
                # Critic은 전역 보상을 예측
                critic_loss = F.mse_loss(V_new, r_mb)

                # === Combined loss ===
                # Actor와 Critic을 함께 학습 (option 1)
                # 또는 별도로 학습 (option 2) - 여기서는 별도 학습
