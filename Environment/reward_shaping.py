...
class Shaping: 
    """
    Reward shaping: r' = a*r - λ(r-μ)^2
    전역 scalar 보상에만 적용 (per-sample shaping은 필요시 shape_batch 사용)
    """
    def __init__(
        self,
        *,
    ...
    def shape_batch(self, fold: int, imm_batch: torch.Tensor) -> torch.Tensor:
        """
        배치 shaping (per-sample 보상용, 선택사항)
        imm_batch: [k] - 선택된 샘플들의 immediate rewards
        returns: [k] - shaped rewards
        """
